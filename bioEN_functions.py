...
def bioen(sim_map_array,exp_map_mask,mask,std,theta,g0,g_init,sf_start,n_iter,epsilon,pgtol,maxiter):
    """
    ...
    g = g_init
    sf_opt = sf_start
    sim_map_array_mask = np.array([map[mask] for map in sim_map_array])
    sim_map_array_mask_sf = sim_map_array_mask * sf_start
    N = np.shape(sim_map_array)[0]
    w0 = np.ones(N)/N
    chisqrt = chiSqrTerm(w0,std,sim_map_array_mask_sf,exp_map_mask)
    ...
    for i in range(0,n_iter):
        if (i == 0):
            ...
            chisqrt = chiSqrTerm(w_opt,std,sim_map_array_mask_sf,exp_map_mask)
...
def bioen_sigma_one(sim_map_array,exp_map_mask,mask,std,theta,g0,g_init,sf_start,sigma_start,n_iter,epsilon,pgtol,maxiter):
    """
    "" Single run of BioEN - with one theta value
    """
    print("THETA = "+str(theta))
...
def bioen_sigma(sim_map_array,exp_map_mask,mask,std,theta,g0,g_init,sf_start,sigma_start,resolution,n_iter,epsilon,pgtol,maxiter):
    """
    "" Single run of BioEN - with one theta value
    """
    print("THETA = "+str(theta))
    ...
    for i in range(0,n_iter):
        if (i == 0):
            ...
            sigma_ = minimize(sigma_fit, [sigma_min], args=(w_opt,std, sim_map_array*sf_opt,exp_map_mask,mask),bounds=bounds,method='L-BFGS-B').x[0]
            sigma_opt = sigma_
            # geting new sim data with new nuisance parameter
            sim_map_g = gaussian_filter(sim_map_array*sf_opt,sigma_opt)
            sim_map_g_mask = np.array([map[mask] for map in sim_map_g])
            chisqrt = chiSqrTerm(w_opt,std,sim_map_g_mask,exp_map_mask)
            print("ChiSqrt = "+str(chisqrt))
        else:
